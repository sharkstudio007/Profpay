...
export default function App() {
  const [groups, setGroups] = useState<Group[]>(() => {
    const saved = localStorage.getItem('profpay_groups');
    if (saved) {
      try {
        ...
        return parsed.map((g: any) => ({
          ...
          students: g.students.map((s: any) => ({
            id: s.id,
            name: s.name,
            attendance: s.attendance || (s.attendedCount ? Array.from({ length: s.attendedCount }).map(() => ({ id: crypto.randomUUID(), date: new Date().toISOString() })) : []),
            payments: s.payments || (s.paidCount ? Array.from({ length: Math.ceil(s.paidCount / 4) }).map(() => ({ id: crypto.randomUUID(), date: new Date().toISOString(), amount: 80, sessionsCount: 4 })) : []),
          }))
        }));
      } catch (e) {
        return [];
  ...
  const [pricePerBlock, setPricePerBlock] = useState<number>(() => {
    const saved = localStorage.getItem('profpay_price');
  ...
  useEffect(() => {
    localStorage.setItem('profpay_groups', JSON.stringify(groups));
  }, [groups]);

  useEffect(() => {
    localStorage.setItem('profpay_price', pricePerBlock.toString());
  }, [pricePerBlock]);
  ...
  const addStudent = (groupId: string, e: React.FormEvent) => {
    e.preventDefault();
  ...
  const addAttendance = (groupId: string, studentId: string) => {
    setGroups(
      groups.map((g) => {
        if (g.id === groupId) {
          return {
            ...
            students: g.students.map((s) =>
              s.id === studentId ? { 
                ...s, 
                attendance: [...s.attendance, { id: crypto.randomUUID(), date: new Date().toISOString() }] 
              } : s
            ),
          };
        }
  ...
  const removeAttendance = (groupId: string, studentId: string) => {
    setGroups(
      groups.map((g) => {
        if (g.id === groupId) {
          return {
            ...
            students: g.students.map((s) => {
              if (s.id === studentId && s.attendance.length > 0) {
                ...
                newAttendance.pop(); // Remove the last record
  ...
  const addBlockPayment = (groupId: string, studentId: string) => {
    setGroups(
      groups.map((g) => {
        if (g.id === groupId) {
          return {
      ...
      <div className="min-h-screen bg-[#050505] flex items-center justify-center p-6 font-sans selection:bg-white selection:text-black">
        ...
        >
          ...
          <div className="bg-white/5 border border-white/10 rounded-[2.5rem] p-8 shadow-2xl backdrop-blur-sm relative overflow-hidden">
            ...
            <form onSubmit={(e) => { e.preventDefault(); setIsLoggedIn(true); }} className="space-y-6 relative z-10">
              ...
              <button 
                type="submit"
                className="w-full bg-white text-black font-bold py-4 rounded-2xl hover:bg-slate-200 transition-all shadow-lg active:scale-[0.98] mt-2 flex items-center justify-center gap-2"
              >
                Accéder au tableau <ChevronRight size={18} />
              </button>
            </form>
    ...
    <div className="min-h-screen neon-white-bg font-sans text-white flex flex-col lg:flex-row">
      ...
      <aside className="w-full lg:w-64 bg-[#0A0A0A] border-b lg:border-b-0 lg:border-r border-white/10 p-6 flex flex-col gap-8 sticky top-0 z-20 lg:h-screen">
        ...
        <nav className="flex flex-row lg:flex-col gap-1 overflow-x-auto lg:overflow-x-visible pb-2 lg:pb-0">
          ...
          <button 
            onClick={() => setIsLoggedIn(false)}
            className="flex lg:hidden items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium transition-all text-red-500 hover:bg-red-500/10"
          >
            <Trash2 size={18} />
            Déconnexion
          </button>
            ...
            <header className="flex flex-col md:flex-row md:items-center justify-between gap-6">
              ...
              <div className="relative w-full md:w-72">
                ...
                <input
                  ...
                  className="w-full bg-white/5 border border-white/10 rounded-xl pl-11 pr-4 py-3 text-sm text-white placeholder:text-slate-600 focus:outline-none focus:ring-2 focus:ring-white/20 focus:border-transparent transition-all"
                />
              </div>
            </header>
